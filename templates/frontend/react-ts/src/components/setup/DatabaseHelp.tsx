...
interface DatabaseHelpProps {
  status: SetupStatus;
}

export function DatabaseHelp({ status }: DatabaseHelpProps) {
  ...
  const envVar = status.requiredEnvVar || (status.databaseEngine === "MongoDB" ? "MONGODB_URI" : "DATABASE_URL");
